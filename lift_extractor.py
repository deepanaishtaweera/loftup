...
from upsamplers import load_loftup_checkpoints
...
class ViTLiFTExtractor(nn.Module):
    def __init__(self, model_type: str = 'dino_vits8', lift_path: str = None, channel: int = 768, patch: int = 8, stride: int = 8,
            layer: int = 11, facet: str = 'key', model: nn.Module = None, device: str = 'cuda', silent=False, upsample_path=None):
        ...
        if upsample_path is not None:
            self.upsample = True
            if "dinov2s" in model_type:
                dim = 384
            elif "dinov2b" in model_type:
                dim = 768
            elif "maskclip" in model_type:
                dim = 512
            elif "siglip" in model_type:
                dim = 768
            else:
                raise ValueError(f"Unknown model type {model_type} for upsampling")
            self.upsampler = load_loftup_checkpoints(upsample_path, dim)
            self.upsampler.to(device)

            # self.featup_upsampler = torch.hub.load("mhamilton723/FeatUp", 'dinov2').to(device)
    ...
    def extract_descriptors(self, batch):
        ...
        fs = self.extractor.extract_descriptors_simple(batch, self.layer, self.facet)
        if self.lift is not None:
            fs = self.lift(batch, fs)
        else:
            if self.upsample:
                # fs = F.interpolate(fs, size=(batch.shape[-2]//2, batch.shape[-1]//2), mode='bilinear', align_corners=False)

                batch = F.interpolate(batch, size=(batch.shape[-2]//2, batch.shape[-1]//2), mode='bilinear', align_corners=False)
                with torch.no_grad():
                    fs = self.upsampler(self.channelnorm(fs), batch)

            fs = fs.reshape(fs.shape[0], fs.shape[1], -1)
            fs = fs.permute(0, 2, 1)
        return fs
