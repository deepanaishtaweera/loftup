...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, patch_output: bool = False, all_layers: bool = False):
        _, _, w, h = x.shape
        ...
        if patch_output:
            *layers, last_resblock = self.transformer.resblocks
            # all_layer_features = [x.permute(1, 0, 2)[:, 1:, :]]  # LND -> NLD
            all_layer_features = []
            if all_layers:
                for layer in layers:
                    x = layer(x)
                    all_layer_features.append(x.permute(1, 0, 2)[:, 1:, :])  # LND -> NLD
            else:
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width)) ## For text encoder
        ...
        self.visual.transformer.resblocks[-2].register_forward_hook(hook_fn_forward)
        
        # MaskCLIP specific parameters
        self.text_channels = 512
    ...
    def get_patch_encodings_all_layers(self, image) -> list:
        """ Get the encodings for each patch in the image for all layers """
        return self.visual(image.type(self.dtype), patch_output=True, all_layers=True)
    ...
    # self._positional_embd = nn.Parameter(self.visual.positional_embedding.data.clone())
    def get_visual_positional_embedding(self) -> nn.Parameter:
        """ Get the visual positional embedding """
        self._positional_embd = nn.Parameter(self.visual.positional_embedding.data.clone())
        return self._positional_embd
    ...
    def extract_feat(self, inputs):
        ...
        pos_embed = self.get_visual_positional_embedding()

        input_imgs = inputs.clone()
        B, C, H, W = inputs.shape
        hw_shape = (H // self.patch_size, W // self.patch_size)
        x_len, pos_len = hw_shape[0]*hw_shape[1], pos_embed.shape[0]

        if x_len != pos_len:
            if pos_len == (H // self.patch_size) * (W // self.patch_size) + 1:
                pos_h = H // self.patch_size
                pos_w = W // self.patch_size
            else:
                raise ValueError(
                    '{}, {}'.format(x_len, pos_len))

            self.visual.positional_embedding.data = self.resize_pos_embed(
                pos_embed[None], hw_shape,  (pos_h, pos_w), 'bicubic')[0]

        inputs = inputs.half()
        _ = self.visual(inputs) ## This is to adapt the new CLIP ViT forward function in ProxyCLIP repo
        v = self.hook_features["v"]
        v = self.extract_v(v, self.visual.transformer.resblocks[-1]).permute(1, 0, 2)
        v = self.visual.ln_post(v)
        v = v[:, 1:]
        v = v.reshape(B, hw_shape[0], hw_shape[1], -1).permute(0, 3, 1, 2).contiguous()
        
        self.visual.positional_embedding.data = pos_embed
        return v, input_imgs
    ...
    def proj_v(self, v):
        proj = nn.Conv2d(self.in_channels, self.text_channels, 1, bias=False)
        proj.weight = nn.Parameter(self.visual.proj.t()[:, :, None, None])
        return proj(v)
    
    def forward_maskclip(self, inputs):
        x, input_imgs = self.extract_feat(inputs)
        feat = self.proj_v(x)
        return feat
